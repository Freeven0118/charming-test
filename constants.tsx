...
export const EXPERT_CONFIG = {
  ...
  // 修改重點：加入 ** 標記以啟用高亮顯示
  description: "看完成績單，你知道問題在哪了。但**「知道」不等於「做到」**。如果你不想繼續盲目摸索，我把這幾年的教學與實戰經驗，濃縮成了五週、總共10小時的系統化訓練。這不僅是外表的升級，更是**「吸引力開關」的啟動**。\n\n我會帶你打造出**「讓女生不自覺多看一眼」的強大氣場**，讓你不再需要費力討好，而是能夠自信地通過擇偶視覺門檻，穩穩拿下戀愛的入場券。",
  // 修改重點：更強烈的行動召喚，暗示這是「解決方案」
  ctaButtonText: "查看 5 週變身計畫",
  ctaButtonSubText: "（每月僅收3人）"
};
...
export const CATEGORY_INFO: Record<Category, { description: string; suggestions: Record<'紅燈' | '黃燈' | '綠燈', string> }> = {
  ...
  '社群形象': {
    description: '在網路上的第一印象。頭貼、照片組合、版面乾淨度，能不能讓人一點進來就知道你是誰、值不值得多看一眼。',
    suggestions: {
